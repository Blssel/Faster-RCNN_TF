...
def get_person_area(im,cls,dets,ax,image_name,video_name,thresh=0.5):
    ...
    if cls!='person':
        ...
        #print('--------------------------------------------------------------------------------------------------------')
        if os.path.exists(os.path.join('/home/yzy/dataset/hmdb-det-part/jump',video_name))==False:
            os.mkdir(os.path.join('/home/yzy/dataset/hmdb-det-part/jump',video_name))
        save_path=os.path.join('/home/yzy/dataset/hmdb-det-part/jump',video_name,image_name)
...
def demo(sess, net, image_name,video_name):
    ...
    im_file = os.path.join('/home/myn/desktop/temporal-segment-networks/hmdb51', video_name, image_name)
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]): #对类中的每一项，获得索引和其类名
        ...
        get_person_area(im2,cls,dets,ax,image_name,video_name,thresh=0.8)
...
def vid_level_det(video_name):
    ...
    FRAME_PATH='/home/myn/desktop/temporal-segment-networks/hmdb51'
    #先获得视频路径
    ...
    for frame in frame_list:
        ...
        #检测后保存至另一个文件夹中
        demo(sess, net, im_name,video_name)
    #num_thread-=1
...
def main():
	...
	sess.graph.finalize()
    	...
    	FRAME_PATH='/home/myn/desktop/temporal-segment-networks/hmdb51'
        test_video_list=[]
        with open('tools/jump_test_split1.txt','r') as f:
            all_video_list=f.readlines()
            for x in all_video_list:
                xx=x.strip().split(' ')
                if xx[1]=='2':
                    test_video_list.append(xx[0].split('.')[0])
